...
resource "aws_lambda_function" "scaffold_webhook" {
  ...
  #layers = ["${aws_lambda_layer_version.scaffold_webhook_layer.arn}"]
}

#resource "aws_lambda_layer_version" "scaffold_webhook_layer" {
...
output "function_name" {
  ...
  value = aws_lambda_function.scaffold_webhook.function_name
}
